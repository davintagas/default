...
function index()
	-- luci 23.05+ has registered the menu via menu.d JSON, no need to re-register
	if not nixio.fs.access("/usr/share/luci/menu.d/luci-app-adguardhome.json") then
		...
		entry({"admin", "services", "AdGuardHome", "overview"},
			cbi("AdGuardHome/overview"), _("Overview"), 1).leaf = true
		entry({"admin", "services", "AdGuardHome", "base"},
			cbi("AdGuardHome/base"), _("Base Settings"), 2).leaf = true
		entry({"admin", "services", "AdGuardHome", "tools"},
			cbi("AdGuardHome/tools"), _("Tools"), 3).leaf = true
		entry({"admin", "services", "AdGuardHome", "log"},
			form("AdGuardHome/log"), _("Log"), 4).leaf = true
		entry({"admin", "services", "AdGuardHome", "manual"},
			cbi("AdGuardHome/manual"), _("Manual Config"), 5).leaf = true
	end

	-- API routes must be registered in both new and old versions
...
function do_update()
	...
	if fs.access("/var/run/update_core") then
		if arg == "force" then
			-- Security fix: script path is a constant, arg can only be "force" or "", no injection risk
